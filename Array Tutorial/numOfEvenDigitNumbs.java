...
public class numOfEvenDigitNumbs {
	public static void main(String args[]) {
		int[] array = { 555, 901, 482, 1771 };
		checkNums(array);
	}

	public static int checkNums (int[] array) {
		int count = 0;
		for (int i = 0; i < array.length; i++) {
			String currentNumAsString = Integer.toString(array[i]);
			if (currentNumAsString.length() % 2 == 0) {
				count++;
			}
		}
		return count;
